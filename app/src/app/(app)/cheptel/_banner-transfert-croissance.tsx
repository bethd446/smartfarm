...
export function BannerTransfertCroissance({ count }: { count: number }) {
  ...
  const handleTransferAll = () => {
    startTransition(async () => {
      ...
      if (res.ok) {
        toast.success(`Transfert réussi : ${res.transfered} porcelet(s) transféré(s) vers Croissance`)
      ...
      <AlertDialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Transférer {count} porcelet{count > 1 ? 's' : ''} vers le bâtiment Croissance ?
              Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isPending}>Annuler</AlertDialogCancel>
            <AlertDialogAction onClick={handleTransferAll} disabled={isPending}>
