-- ============================================================
-- Migration 20260523000000_kpi_ifip_productivite.sql
-- D4-A : KPI Productivité IFIP — MCA + IC ferme + GMQ par stade
--        + Règles R27 (IC>3.2) et R28 (GMQ engraissement<600g/j)
-- ============================================================
-- Référentiel IFIP :
--   MCA = (Coût alimentaire € / kg de croît produit)
--   IC  = (kg aliment consommé / kg croît) — cible 2.6-2.8 excellent, >3.2 à revoir
--   GMQ = gain moyen quotidien (g/j) — engraissement cible 750-850
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- Indexes auxiliaires (idempotents)
-- ------------------------------------------------------------
CREATE INDEX IF NOT EXISTS idx_conso_bande_date
  ON consommations_aliment(bande_id, date);
CREATE INDEX IF NOT EXISTS idx_pesees_bande_date
  ON pesees(bande_id, date_pesee);
CREATE INDEX IF NOT EXISTS idx_pesees_animal_date
  ON pesees(animal_id, date_pesee);

-- ============================================================
-- VUE 1 : v_kpi_mca_ferme
-- Marge sur Coût Alimentaire par ferme (XOF/kg croît).
-- ============================================================
...
WITH conso AS (
  SELECT
    b.ferme_id,
    SUM(c.quantite_kg)::numeric         AS conso_total_kg,
    SUM(COALESCE(c.cout, 0))::numeric   AS cout_alim_total_xof,
    MIN(c.date)                         AS periode_debut,
    MAX(c.date)                         AS periode_fin
  FROM consommations_aliment c
  JOIN bandes b ON b.id = c.bande_id AND b.deleted_at IS NULL
  GROUP BY b.ferme_id
),
croit AS (
  -- Croît total = somme des (poids dernière pesée - poids naissance/entrée)
  -- par animal présent dans la ferme.
  SELECT
    a.ferme_id,
    SUM(GREATEST(p.dernier_poids - COALESCE(a.poids_naissance_kg, 0), 0))::numeric AS croit_total_kg
  FROM animaux a
  JOIN LATERAL (
    SELECT poids_kg AS dernier_poids
    FROM pesees pp
    WHERE pp.animal_id = a.id
      AND pp.deleted_at IS NULL
      AND pp.type = 'individuelle'
    ORDER BY pp.date_pesee DESC
    LIMIT 1
  ) p ON true
...
SELECT
  COALESCE(c.ferme_id, k.ferme_id)               AS ferme_id,
  c.periode_debut,
  c.periode_fin,
  COALESCE(c.conso_total_kg, 0)                  AS conso_total_kg,
  COALESCE(c.cout_alim_total_xof, 0)             AS cout_alim_total_xof,
  COALESCE(k.croit_total_kg, 0)                  AS croit_total_kg,
  CASE
    WHEN COALESCE(k.croit_total_kg, 0) > 0
      THEN ROUND(c.cout_alim_total_xof / k.croit_total_kg, 2)
    ELSE NULL
  END                                            AS mca_xof_par_kg,
  CASE
    WHEN COALESCE(c.conso_total_kg, 0) > 0
      THEN ROUND(c.cout_alim_total_xof / c.conso_total_kg, 2)
    ELSE NULL
  END                                            AS prix_aliment_moyen_xof_kg
FROM conso c
FULL OUTER JOIN croit k ON k.ferme_id = c.ferme_id;

GRANT SELECT ON v_kpi_mca_ferme TO anon, authenticated;

-- ============================================================
-- VUE 2 : v_kpi_ic_ferme
...
WITH conso AS (
  SELECT
    b.ferme_id,
    SUM(c.quantite_kg)::numeric AS conso_kg,
    MIN(c.date)                 AS periode_debut,
    MAX(c.date)                 AS periode_fin
  FROM consommations_aliment c
  JOIN bandes b ON b.id = c.bande_id AND b.deleted_at IS NULL
...
croit AS (
  SELECT
    a.ferme_id,
    SUM(GREATEST(p.dernier_poids - COALESCE(a.poids_naissance_kg, 0), 0))::numeric AS croit_kg
  FROM animaux a
  JOIN LATERAL (
    SELECT poids_kg AS dernier_poids
    FROM pesees pp
...
SELECT
  COALESCE(c.ferme_id, k.ferme_id)               AS ferme_id,
  CASE
    WHEN COALESCE(k.croit_kg, 0) > 0
      THEN ROUND(c.conso_kg / k.croit_kg, 2)
    ELSE NULL
  END                                            AS ic,
  COALESCE(c.conso_kg, 0)                        AS conso_kg,
  COALESCE(k.croit_kg, 0)                        AS croit_kg,
  c.periode_debut,
  c.periode_fin
FROM conso c
FULL OUTER JOIN croit k ON k.ferme_id = c.ferme_id;

GRANT SELECT ON v_kpi_ic_ferme TO anon, authenticated;

-- ============================================================
-- VUE 3 : v_kpi_gmq_par_stade
-- GMQ par stade de vie : porcelet (0-28j), sevrage (28-70j), engraissement (>70j).
-- ============================================================
...
WITH pesees_age AS (
  SELECT
    ...
    p.date_pesee,
    p.poids_kg,
    (p.date_pesee - a.date_naissance)::int AS age_j,
    CASE
      WHEN (p.date_pesee - a.date_naissance) <  28  THEN 'porcelet'
      WHEN (p.date_pesee - a.date_naissance) <  70  THEN 'sevrage'
      ELSE 'engraissement'
    END               AS stade
  FROM pesees p
  JOIN animaux a       ON a.id = p.animal_id AND a.deleted_at IS NULL
  LEFT JOIN bande_animaux ba ON ba.animal_id = a.id
                            AND (ba.date_sortie IS NULL OR ba.date_sortie >= p.date_pesee)
  WHERE p.deleted_at IS NULL
    AND p.type = 'individuelle'
    AND a.date_naissance IS NOT NULL
    AND p.date_pesee >= a.date_naissance
),
gmq_calc AS (
  -- Pour chaque (animal, stade) on prend première et dernière pesée et on calcule GMQ
  SELECT
    ferme_id,
    bande_id,
    animal_id,
    stade,
    MIN(date_pesee) AS d_min,
    MAX(date_pesee) AS d_max,
    MIN(age_j)      AS age_min_j,
    MAX(age_j)      AS age_max_j,
    (
      SELECT poids_kg FROM pesees_age pa2
      WHERE pa2.animal_id = pa.animal_id AND pa2.stade = pa.stade
      ORDER BY pa2.date_pesee ASC LIMIT 1
    )               AS poids_min_kg,
    (
      SELECT poids_kg FROM pesees_age pa2
      WHERE pa2.animal_id = pa.animal_id AND pa2.stade = pa.stade
      ORDER BY pa2.date_pesee DESC LIMIT 1
    )               AS poids_max_kg
  FROM pesees_age pa
  GROUP BY ferme_id, bande_id, animal_id, stade
  HAVING COUNT(*) >= 2 AND (MAX(date_pesee) - MIN(date_pesee)) > 0
)
SELECT
  ferme_id,
  bande_id,
  stade,
  ROUND(
    AVG( (poids_max_kg - poids_min_kg) * 1000.0 / NULLIF((d_max - d_min), 0) ),
    1
  )::numeric                          AS gmq_g_par_jour,
  COUNT(*)::int                       AS nb_pesees,
  MIN(age_min_j)                      AS age_min_j,
  MAX(age_max_j)                      AS age_max_j
FROM gmq_calc
GROUP BY ferme_id, bande_id, stade;

GRANT SELECT ON v_kpi_gmq_par_stade TO anon, authenticated;

-- ============================================================
-- v_alertes_actives : RECRÉATION avec R01-R26 préservés byte-identique
-- + ajout R27 (IC>3.2) et R28 (GMQ engraissement<600g/j)
-- ============================================================
...
UNION ALL
-- ------------------------------------------------------------
-- R27 : IC ferme > 3.2 sur période ≥ 30 jours (nutrition, moyenne)
-- ------------------------------------------------------------
SELECT
  'R27-ic-ferme-degrade'::text                                                     AS regle_id,
  'ferme'::text                                                                    AS cible_type,
  f.id::text                                                                       AS cible_id,
  f.nom                                                                            AS cible_label,
  'moyenne'::text                                                                  AS gravite,
  ('IC ferme '::text || f.nom || ' = ' || ic.ic::text
   || ' (cible IFIP 2.6-2.8, alerte > 3.2)')::text                                 AS titre,
  ('Indice de consommation dégradé sur '
   || (ic.periode_fin - ic.periode_debut)::text
   || ' jours : '|| ic.conso_kg::text || ' kg aliment pour '
   || ic.croit_kg::text || ' kg croît. Revoir formulation, gaspillage, sanitaire.'
  )::text                                                                          AS description,
...
WHERE ic.ic IS NOT NULL
  AND ic.ic > 3.2
  AND ic.periode_debut IS NOT NULL
  AND ic.periode_fin   IS NOT NULL
  AND (ic.periode_fin - ic.periode_debut) >= 30

UNION ALL
-- ------------------------------------------------------------
-- R28 : GMQ engraissement < 600 g/j (nutrition, moyenne)
--       Cible IFIP 750-850 g/j
-- ------------------------------------------------------------
SELECT
  'R28-gmq-engraissement-bas'::text                                                AS regle_id,
  'bande'::text                                                                    AS cible_type,
  COALESCE(b.id, g.bande_id)::text                                                 AS cible_id,
  COALESCE(b.code, 'bande-' || LEFT(g.bande_id::text, 8))                          AS cible_label,
  'moyenne'::text                                                                  AS gravite,
  ('GMQ engraissement '
   || COALESCE(b.code, 'bande')
   || ' = ' || g.gmq_g_par_jour::text || ' g/j (cible IFIP 750-850)')::text        AS titre,
  ('Gain moyen quotidien sous le seuil critique de 600 g/j sur '
   || g.nb_pesees::text || ' pesée(s). Vérifier formulation finition, '
   || 'densité bâtiment, accès eau, sanitaire.'
  )::text                                                                          AS description,
  ('/bandes/' || COALESCE(b.id, g.bande_id)::text)::text                           AS lien_suggere,
  now()                                                                            AS detecte_le,
  g.ferme_id                                                                       AS ferme_id
FROM v_kpi_gmq_par_stade g
LEFT JOIN bandes b ON b.id = g.bande_id AND b.deleted_at IS NULL
WHERE g.stade = 'engraissement'
  AND g.gmq_g_par_jour IS NOT NULL
  AND g.gmq_g_par_jour < 600
;
