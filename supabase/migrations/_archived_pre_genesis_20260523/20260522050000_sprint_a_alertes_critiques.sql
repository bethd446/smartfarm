-- =====================================================================
-- Sprint A — 4 règles critiques R23-R26 + vue v_densite_batiment
-- =====================================================================
-- Sources : INRAE (vermifuge truie J-14 pré-MB), CIRAD (Fer porcelet
-- anémie tropicale), IFIP (BCS sevrage), FAO/CIRAD (densité bâtiment).
--
-- - R23 : Truie gestante (saillie + diag positif, MB prévue dans 0-14j)
--         sans vermifuge récent (<= 30j).
-- - R24 : Mise-bas avec porcelets J3-J7 sans Fer dextran administré.
-- - R25 : Sevrage avec bcs_truie < 2.5 sans nouvelle saillie depuis.
-- - R26 : Bâtiment dont taux d'occupation >= 95 %.
--
-- ⚠️ La table traitements utilise (date_debut, date_fin) et non
--    date_traitement. Adapté en conséquence.
-- =====================================================================

BEGIN;

-- ---------------------------------------------------------------------
-- 1. Vue v_densite_batiment (utilisée par R26 + page /batiments future)
-- ---------------------------------------------------------------------
DROP VIEW IF EXISTS v_densite_batiment CASCADE;

CREATE VIEW v_densite_batiment
...
COMMENT ON VIEW v_densite_batiment IS
  'Effectif actuel + taux d''occupation par bâtiment. Référentiel FAO/CIRAD pour seuil 95 %.';

-- ---------------------------------------------------------------------
-- 2. Recréer v_alertes_actives : 22 branches existantes + 4 nouvelles
-- ---------------------------------------------------------------------
    ...
    AND NOT EXISTS (
      SELECT 1 FROM traitements t
       WHERE t.animal_id = a.id
         ...
         AND t.date_debut >= CURRENT_DATE - 30
         AND t.deleted_at IS NULL
    )
UNION ALL
-- =====================================================================
-- R24 — Fer porcelet J3 non administré (CIRAD, anémie tropicale)
-- =====================================================================
    ...
    AND NOT EXISTS (
      SELECT 1 FROM traitements t
       WHERE (t.bande_id = mb.bande_id OR t.animal_id = mb.truie_id)
         ...
         AND t.date_debut >= mb.date_mise_bas
         AND t.deleted_at IS NULL
    )
UNION ALL
-- =====================================================================
...
UNION ALL
-- =====================================================================
-- R26 — Surdensité bâtiment >= 95 % (FAO/CIRAD)
-- =====================================================================
...
COMMENT ON VIEW v_alertes_actives IS
  '26 règles d''alertes actives (R01-R26). Mapping UI: src/lib/alertes-regles.ts.';
